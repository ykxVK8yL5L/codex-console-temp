    ...
    try:
        ...
        with get_db() as db:
            for attr_name, defn in SETTING_DEFINITIONS.items():
                ...
                if not existing:
                ...
                elif attr_name == "log_retention_days" and str(existing.value or "").strip() == "30":
                    upgraded_value = _value_to_string(defn.default_value)
                    set_setting(
                        db,
                        defn.db_key,
                        upgraded_value,
                    ...
                    print(f"[Settings] 升级默认设置: {defn.db_key} 30 -> {upgraded_value}")
