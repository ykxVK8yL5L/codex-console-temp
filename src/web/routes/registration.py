...
def _cleanup_stale_outlook_account_claims(now_ts: Optional[float] = None) -> None:
    ...
    for email, meta in list(_OUTLOOK_ACCOUNT_CLAIMS.items()):
        ...
        if (
            claimed_at
            and (now_value - claimed_at) <= _OUTLOOK_ACCOUNT_CLAIM_TTL_SECONDS
        ):
            continue
...
def _release_outlook_account(
    email: Optional[str], task_uuid: Optional[str] = None
) -> None:
    email_norm = _normalize_outlook_account_email(email)
    if not email_norm:
        return
...
def recover_interrupted_registration_tasks() -> Dict[str, int]:
    ...
    with get_db() as db:
        stale_running_tasks = (
            ...
            .filter(RegistrationTask.status == "running")
            .all()
        )
        stale_pending_tasks = (
            db.query(RegistrationTask)
            .filter(RegistrationTask.status == "pending")
            .all()
        )
...
def _get_task_recovery_reason(task: RegistrationTask) -> Optional[str]:
    ...
    if (
        ...
        and message == RECOVERED_RUNNING_TASK_ERROR
    ):
        return "startup_recovered_running"
    if (
        str(getattr(task, "status", "") or "") == "cancelled"
        and message == RECOVERED_PENDING_TASK_ERROR
    ):
...
def scheduled_job_to_response(
    job: ScheduledRegistrationJob,
) -> ScheduledRegistrationJobResponse:
    """转换计划任务模型为响应"""
...
def _normalize_email_service_config(
    ...
    if service_type == EmailServiceType.MOE_MAIL:
        ...
        if (
            ...
            and "admin_password" not in normalized
        ):
...
def _should_skip_auto_upload_for_token_profile(
    ...
    if not filter_only_access_token_accounts:
        return False
    return (
        _normalize_token_completeness(token_metadata.get("token_completeness"))
        == TOKEN_COMPLETENESS_ONLY_ACCESS
    )
    ...
    with get_db() as db:
        ...
        try:
            ...
            task = crud.update_registration_task(
                db, task_uuid, status="running", started_at=utcnow_naive()
            )
            ...
            bound_email_service_id = email_service_id or getattr(
                task, "email_service_id", None
            )
            if bound_email_service_id:
                ...
                if db_service:
                    ...
                    logger.info(
                        f"使用数据库邮箱服务: {db_service.name} (ID: {db_service.id}, 类型: {service_type.value})"
                    )
                    if service_type == EmailServiceType.OUTLOOK:
                        outlook_email = _normalize_outlook_account_email(
                            (db_service.config or {}).get("email")
                        )
                        if not outlook_email:
                            raise ValueError(
                                f"Outlook 账户缺少邮箱配置: {db_service.id}"
                            )
                        if not _claim_outlook_account(
                            outlook_email, task_uuid, db_service.id
                        ):
                            logger.warning(
                                f"Outlook 账户已被当前批次占用，无法重复使用: {outlook_email}"
                            )
                            raise ValueError(
                                f"Outlook 账户已被当前批次占用: {outlook_email}"
                            )
                        claimed_outlook_email = outlook_email
                else:
                    raise ValueError(
                        f"邮箱服务不存在或已禁用: {bound_email_service_id}"
                    )
            else:
                if service_type == EmailServiceType.TEMPMAIL:
                    if not settings.tempmail_enabled:
                        raise ValueError(
                            "Tempmail.lol 渠道已禁用，请先在邮箱服务页面启用"
                        )
                    config = {
                    ...
                    if not settings.yyds_mail_enabled or not api_key:
                        raise ValueError(
                            "YYDS Mail 渠道未启用或未配置 API Key，请先在邮箱服务页面配置"
                        )
                    config = {
                    ...
                    if db_service and db_service.config:
                        ...
                        settings.custom_domain_base_url
                        and settings.custom_domain_api_key
                    ):
                        config = {
                    ...
                    for svc in outlook_services:
                        ...
                        existing = (
                            db.query(Account)
                            .filter(func.lower(Account.email) == email_norm)
                            .first()
                        )
                        if existing:
                        ...
                        if not _claim_outlook_account(email_norm, task_uuid, svc.id):
                            logger.info(
                                f"Outlook 账户已被当前批次占用，跳过: {email_norm}"
                            )
                            continue
                    ...
                    if selected_service and selected_service.config:
                        ...
                        crud.update_registration_task(
                            db, task_uuid, email_service_id=selected_service.id
                        )
                        logger.info(f"使用数据库 Outlook 账户: {selected_service.name}")
                    else:
                        raise ValueError(
                            "所有 Outlook 账户都已注册过 OpenAI 账号，请添加新的 Outlook 账户"
                        )
                    ...
                    if db_service and db_service.config:
                        ...
                        logger.info(
                            f"使用数据库 Temp-Mail 自部署服务: {db_service.name}"
                        )
                    else:
                        config = _normalize_email_service_config(
                            service_type, email_service_config or {}, actual_proxy_url
                        )
                        missing_keys = [
                            key
                            for key in ("base_url", "admin_password", "domain")
                            if not config.get(key)
                        ]
                        if missing_keys:
                            raise ValueError(
                                "没有可用的 Temp-Mail 自部署服务，请先在邮箱服务页面添加服务"
                            )
                    ...
                    if db_service and db_service.config:
                        ...
                        if missing_keys:
                            raise ValueError(
                                "没有可用的 CloudMail 服务，请先在邮箱服务页面添加服务"
                            )
                    ...
                    if db_service and db_service.config:
                        ...
                        raise ValueError(
                            "没有可用的 DuckMail 邮箱服务，请先在邮箱服务页面添加服务"
                        )
                    ...
                    if db_service and db_service.config:
                        ...
                        raise ValueError(
                            "没有可用的 Freemail 邮箱服务，请先在邮箱服务页面添加服务"
                        )
                    ...
                    if db_service and db_service.config:
                        ...
                        if not config.get("api_key"):
                            raise ValueError(
                                "没有可用的 LuckMail 服务，请先在邮箱服务中添加并填写 API Key"
                            )
                else:
            ...
            if result.success:
                ...
                _record_token_profile_stat(
                    single_token_stats, metadata.get("token_completeness")
                )
                ...
                if auto_upload_targets_enabled and skip_auto_upload_for_token_profile:
                    upload_log(
                        "[上传过滤] 当前账号仅拿到 access_token、未拿到 refresh_token，已按配置跳过上传到管理平台"
                    )
                            ...
                            for _sid in _s2a_ids:
                                try:
                                    ...
                                    if not _svc:
                                        continue
                                    upload_log(
                                        f"[Sub2API] 正在把账号发往服务站: {_svc.name}"
                                    )
                ...
                if auto_upload_codex2api and not skip_auto_upload_for_token_profile:
                    try:
                        ...
                        if saved_account and (
                            saved_account.refresh_token or saved_account.access_token
                        ):
                            ...
                            for _sid in _codex2api_ids:
                                try:
                                    ...
                                    if not _svc:
                                        continue
                                    upload_log(
                                        f"[Codex2API] 正在把账号发往服务站: {_svc.name}"
                                    )
                                    _ok, _msg = upload_to_codex2api(
                                        [saved_account], _svc.api_url, _svc.admin_key
                                    )
                                    if _ok:
                                        mark_scheduler_uploaded()
                                    upload_log(
                    ...
                    try:
                        ...
                        if saved_account and saved_account.access_token:
                            ...
                            if not _new_api_ids:
                                _new_api_ids = [
                                    s.id
                                    for s in crud.get_new_api_services(db, enabled=True)
                                ]
                            if not _new_api_ids:
                            ...
                            for _sid in _new_api_ids:
                                try:
                                    ...
                                    if _ok:
                                        mark_scheduler_uploaded()
                                    upload_log(
                                        f"[NewAPI] {'成功' if _ok else '失败'}({_svc.name}): {_msg}"
                                    )
                                except Exception as _e:
                ...
                if (
                    task_manager.is_cancelled(task_uuid)
                    or str(result.error_message or "").strip() == "任务已取消"
                ):
                    _mark_task_cancelled(
                        db, task_uuid, result.error_message or "任务已取消"
                    )
                    ...
                    task_manager.update_status(
                        task_uuid, "failed", error=result.error_message
                    )
                    logger.warning(
...
async def run_registration_task(
    ...
    if task_manager.is_cancelled(task_uuid):
        ...
        task_manager.add_log(
            task_uuid,
            f"{log_prefix} [取消] 任务已取消" if log_prefix else "[取消] 任务已取消",
        )
        return
...
async def run_batch_parallel(
    ...
    add_batch_log(
        f"[系统] 并行模式启动，并发数: {max_workers}，总任务: {len(task_uuids)}"
    )
            ...
            async with counter_lock:
                ...
                if t.status == "completed":
                    ...
                    error_message = t.error_message or (
                        "任务已取消" if t.status == "cancelled" else "未知错误"
                    )
    ...
    async def _worker(worker_index: int):
        ...
        while True:
            if (
                task_manager.is_batch_cancelled(batch_id)
                or batch_tasks[batch_id]["cancelled"]
            ):
                break
            try:
    ...
    try:
        ...
        if (
            ...
            with get_db() as db:
                while not queue.empty():
                    ...
                    task_manager.update_status(
                        remaining_uuid, "cancelled", error="批量任务已取消"
                    )
                    queue.task_done()
        ...
        if not task_manager.is_batch_cancelled(batch_id):
            ...
            if failed_count > 0:
                add_batch_log(
                    f"[完成] 批量任务完成！成功: {success_count}, 未成功: {failed_count}"
                )
            else:
...
async def run_auto_registration_batch(plan, settings: Settings) -> str:
    ...
    if batch:
        ...
        add_auto_registration_log(
            f"[自动注册] {token_profile_summary.replace('[统计] ', '', 1)}"
        )
...
def _validate_registration_request(email_service_type: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"无效的邮箱服务类型: {email_service_type}"
        ) from exc


def _schedule_async_job(
    background_tasks: Optional[BackgroundTasks], coroutine_func, *args
):
    """统一调度后台异步任务。"""
...
async def dispatch_registration_config(
    ...
    if email_service_type == "outlook_batch":
        request = OutlookBatchRegistrationRequest(
            ...
            enable_alias=bool(config.get("enable_alias", False)),
            ...
            filter_only_access_token_accounts=bool(
                config.get("filter_only_access_token_accounts", True)
            ),
        )
        response = await _start_outlook_batch_registration_internal(
...
async def get_registration_stats():
    ...
    with get_db() as db:
        ...
        today_success_rate = (
            round((today_success / today_total) * 100, 1) if today_total > 0 else 0.0
        )
...
async def get_available_email_services():
    ...
    result = {
        ...
        "web2": {"available": False, "count": 0, "services": []},
...
async def get_outlook_accounts_for_registration():
    ...
    with get_db() as db:
        ...
        for service in outlook_services:
            ...
            existing_account = (
                db.query(Account)
                .filter(func.lower(Account.email) == str(email).lower())
                .first()
            )
...
async def list_scheduled_registration_jobs(
    ...
    offset = (page - 1) * page_size
    with get_db() as db:
        jobs = crud.get_scheduled_registration_jobs(
            db, enabled=enabled, skip=offset, limit=page_size
        )
        total_query = db.query(ScheduledRegistrationJob)
        if enabled is not None:
            total_query = total_query.filter(
                ScheduledRegistrationJob.enabled == enabled
            )
...
async def create_scheduled_registration_job(request: ScheduledRegistrationRequest):
    """创建计划注册任务。"""
    now = utcnow_naive()
    normalized_schedule_config = normalize_schedule_config(
        request.schedule_type, request.schedule_config, now
    )
    next_run_at = compute_next_run_at(
        request.schedule_type, normalized_schedule_config, now
    )
...
async def update_scheduled_registration_job(
    job_uuid: str, request: ScheduledRegistrationRequest
):
    """更新计划注册任务。"""
    now = utcnow_naive()
    normalized_schedule_config = normalize_schedule_config(
...
@router.post(
    "/schedules/{job_uuid}/enable", response_model=ScheduledRegistrationJobResponse
)
async def enable_scheduled_registration_job(job_uuid: str):
    ...
    with get_db() as db:
        ...
        if not job:
            raise HTTPException(status_code=404, detail="计划任务不存在")
        next_run_at = compute_next_run_at(
            job.schedule_type, job.schedule_config or {}, now
        )
...
@router.post(
    "/schedules/{job_uuid}/pause", response_model=ScheduledRegistrationJobResponse
)
...
async def run_scheduled_registration_job_now(
    job_uuid: str, background_tasks: BackgroundTasks
):
    """立即执行一次计划注册任务。"""
    now = utcnow_naive()
    with get_db() as db:
        ...
        if not claimed:
            raise HTTPException(
                status_code=409, detail="计划任务状态已变化，请刷新后重试"
            )

    try:
        result = await dispatch_registration_config(
            claimed.registration_config or {}, background_tasks
        )
        with get_db() as db:
