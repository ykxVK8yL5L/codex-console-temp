...
from ...database.session import get_db, get_database_backend, get_database_pool_settings
from ...services import EmailServiceType

logger = logging.getLogger(__name__)
router = APIRouter()

DEFAULT_TASK_CLEANUP_DAYS = 14
DEFAULT_FAILED_TASK_RETENTION_DAYS = 45
DEFAULT_STALE_PENDING_TASK_DAYS = 7
DEFAULT_STALE_RUNNING_TASK_DAYS = 3
...
async def update_webui_settings(request: WebUISettings):
    ...
    if request.access_password:
        ...
        update_dict["webui_secret_key"] = secrets.token_urlsafe(48)
...
async def get_database_info():
    ...
    database_supports_file_ops = database_backend == "sqlite"
    pool_settings = get_database_pool_settings(settings.database_url)

    db_size = 0
    if database_supports_file_ops:
    ...
    return {
        ...
        "database_size_label": f"{round(db_size / (1024 * 1024), 2)} MB" if database_supports_file_ops else "由数据库服务端管理",
        "database_supports_file_backup": database_supports_file_ops,
        "database_supports_file_import": database_supports_file_ops,
...
async def backup_database():
    ...
    if get_database_backend(settings.database_url) != "sqlite":
        raise HTTPException(status_code=400, detail="当前仅支持 SQLite 文件备份，PostgreSQL 请使用 pg_dump")
...
async def cleanup_database(
    days: int = DEFAULT_TASK_CLEANUP_DAYS,
    keep_failed: bool = True,
    failed_days: int = DEFAULT_FAILED_TASK_RETENTION_DAYS,
    stale_pending_days: int = DEFAULT_STALE_PENDING_TASK_DAYS,
    stale_running_days: int = DEFAULT_STALE_RUNNING_TASK_DAYS,
):
    """清理过期数据"""
    ...
    terminal_days = max(1, int(days))
    failed_retention_days = max(terminal_days, int(failed_days if keep_failed else days))
    pending_retention_days = max(1, int(stale_pending_days))
    running_retention_days = max(1, int(stale_running_days))
    now = utcnow_naive()

    terminal_cutoff = now - timedelta(days=terminal_days)
    failed_cutoff = now - timedelta(days=failed_retention_days)
    pending_cutoff = now - timedelta(days=pending_retention_days)
    running_cutoff = now - timedelta(days=running_retention_days)

    with get_db() as db:
        terminal_ref = func.coalesce(
            RegistrationTask.completed_at,
            RegistrationTask.started_at,
            RegistrationTask.created_at,
        )
        running_ref = func.coalesce(RegistrationTask.started_at, RegistrationTask.created_at)
        cleanup_filters = [
            and_(RegistrationTask.status.in_(["completed", "cancelled"]), terminal_ref < terminal_cutoff),
            and_(RegistrationTask.status == "pending", RegistrationTask.created_at < pending_cutoff),
            and_(RegistrationTask.status == "running", running_ref < running_cutoff),
            and_(RegistrationTask.status == "failed", terminal_ref < failed_cutoff),
        ]

        result = db.execute(delete(RegistrationTask).where(or_(*cleanup_filters)))
        db.commit()
    ...
    return {
        "success": True,
        "message": (
            f"已清理 {deleted_count} 条任务记录：已完成/已取消保留 {terminal_days} 天，"
            f"失败保留 {failed_retention_days} 天，待处理保留 {pending_retention_days} 天，"
            f"运行中保留 {running_retention_days} 天"
        ),
        "deleted_count": deleted_count,
        "retention_days": terminal_days,
        "failed_retention_days": failed_retention_days,
        "stale_pending_days": pending_retention_days,
        "stale_running_days": running_retention_days,
    }
