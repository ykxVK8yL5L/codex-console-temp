...
let autoQuickRefreshSettingsTimer = null;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!autoQuickRefreshSettingsTimer) {
        autoQuickRefreshSettingsTimer = setInterval(() => {
            loadAutoQuickRefreshSettings({ silent: true });
        }, 30000);
    }
    initEventListeners();
...
document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'hidden' && autoQuickRefreshSettingsTimer) {
        clearInterval(autoQuickRefreshSettingsTimer);
        autoQuickRefreshSettingsTimer = null;
        return;
    }
    if (document.visibilityState === 'visible' && !autoQuickRefreshSettingsTimer) {
        loadAutoQuickRefreshSettings({ silent: true });
        autoQuickRefreshSettingsTimer = setInterval(() => {
            loadAutoQuickRefreshSettings({ silent: true });
        }, 30000);
                ...
                <div style="display:flex;gap:4px;align-items:center;white-space:nowrap;">
                    ...
                    <div class="dropdown" style="position:relative;">
                        ...
                        <div class="dropdown-menu" style="min-width:100px;">
                            ...
                            <a href="#" class="dropdown-item" onclick="event.preventDefault();closeMoreMenu(this);uploadToCodex2api(${account.id})">传 Codex2API</a>
...
function renderSubscriptionStatus(subscriptionType) {
    const normalized = normalizeSubscriptionType(subscriptionType);
    const variant = normalized === 'plus' ? 'plus' : 'free';
...
async function markSubscription(id) {
    const type = prompt('请输入订阅类型 (plus / free):', 'plus');
    if (!type) return;
    if (!['plus', 'free'].includes(type.trim().toLowerCase())) {
        toast.error('无效的订阅类型，请输入 plus 或 free');
        return;
    }
    try {
