...
const elements = {
    ...
    filterOnlyAccessTokenAccounts: document.getElementById('filter-only-access-token-accounts'),
...
function updateEmailServiceOptions() {
    ...
    if ((availableServices.tempmail && availableServices.tempmail.available) ||
        ...
        (availableServices.web2 && availableServices.web2.available)) {
        ...
        if (availableServices.web2 && availableServices.web2.available) {
            availableServices.web2.services.forEach(service => {
                const option = document.createElement('option');
                option.value = `web2:${service.id}`;
                option.textContent = service.name;
                option.dataset.type = 'web2';
...
function handleServiceChange(e) {
    ...
    if (type === 'outlook') {
        ...
        const service = availableServices.web2.services.find(s => s.id == id);
        if (service) {
            addLog('info', `[系统] 已选择 Web2 邮箱服务: ${service.name}`);
        }
...
function setRegistrationConfigToForm(config) {
    ...
    if (elements.filterOnlyAccessTokenAccounts) {
        elements.filterOnlyAccessTokenAccounts.checked = registrationConfig.filter_only_access_token_accounts !== false;
    }
...
async function loadTodayStats(silent = true) {
    try {
        ...
        const todayByStatus = data?.today_by_status || {};
        const hasTodaySuccess = data?.today_success != null || todayByStatus.completed != null;
        const hasTodayFailed = data?.today_failed != null || todayByStatus.failed != null;
        const success = Number(data?.today_success ?? todayByStatus.completed ?? 0);
        const failed = Number(data?.today_failed ?? todayByStatus.failed ?? 0);
        const fallbackTotal = Number(data?.today_total ?? data?.today_count ?? 0);
        const total = hasTodaySuccess || hasTodayFailed ? (success + failed) : fallbackTotal;
        const safeTotal = Number.isFinite(total) ? total : 0;
        const computedRate = safeTotal > 0 ? (success / safeTotal) * 100 : 0;
        const rate = hasTodaySuccess || hasTodayFailed
            ? computedRate
            : Number(data?.today_success_rate ?? computedRate);
        renderTodayStats(safeTotal, success, failed, Number.isFinite(rate) ? rate : 0);
    } catch (error) {
...
async function restoreActiveTask() {
    ...
    if (mode === 'single' && task_uuid) {
        ...
        try {
            const monitor = await api.get('/registration/auto-monitor');
            const autoEnabled = Boolean(monitor && monitor.enabled);
            if (!autoEnabled) {
                sessionStorage.removeItem('activeTask');
                return;
            }
            ...
            elements.startBtn.disabled = true;
            elements.cancelBtn.disabled = false;
            addLog('info', '[系统] 检测到自动注册监控，正在恢复状态...');
            startAutoRegistrationMonitor();
        } catch {
            sessionStorage.removeItem('activeTask');
        }
...
async function refreshOutlookRegistrationStatus() {
    try {
        ...
        outlookAccounts = data.accounts || [];
        renderOutlookAccountsList();
        addLog('info', `[系统] Outlook 注册状态已刷新 (已注册: ${data.registered_count}, 未注册: ${data.unregistered_count})`);
        toast.success('Outlook 注册状态已刷新');
    } catch (error) {
        console.error('刷新 Outlook 注册状态失败:', error);
        toast.error('刷新 Outlook 注册状态失败: ' + error.message);
    }
}
