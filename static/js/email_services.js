...
const elements = {
    ...
    addWeb2Fields: document.getElementById('add-web2-fields'),
    ...
    editWeb2Fields: document.getElementById('edit-web2-fields'),
...
function switchAddSubType(subType) {
    ...
    elements.addWeb2Fields.style.display = subType === 'web2' ? '' : 'none';
...
function switchEditSubType(subType) {
    ...
    elements.editWeb2Fields.style.display = subType === 'web2' ? '' : 'none';
...
async function loadStats() {
    try {
        ...
        elements.customCount.textContent = (data.custom_count || 0) + (data.tempmail_builtin_count || 0) + (data.yyds_mail_count || 0) + (data.web2_count || 0) + (data.temp_mail_count || 0) + (data.cloudmail_count || 0) + (data.duck_mail_count || 0) + (data.luckmail_count || 0) + (data.freemail_count || 0) + (data.imap_mail_count || 0);
...
function getCustomServiceTypeBadge(subType) {
    ...
    if (subType === 'web2') {
        return '<span class="status-badge" style="background-color:#4f46e5;color:white;">Web2</span>';
    }
    if (subType === 'tempmail') {
...
function getCustomServiceAddress(service) {
    ...
    if (service._subType === 'web2') {
        const baseUrl = service.config?.base_url || 'https://web2.temp-mail.org';
        return `${escapeHtml(baseUrl)}<div style="color: var(--text-muted); margin-top: 4px;">固定代理：127.0.0.1:7890</div>`;
    }
...
async function loadCustomServices() {
    try {
        const [r1, r2, r3, r4, r5, r6, r7, r8, r9, r10] = await Promise.all([
            ...
            api.get('/email-services?service_type=web2'),
        ...
        customServices = [
            ...
            ...(r4.services || []).map(s => ({ ...s, _subType: 'web2' })),
            ...(r5.services || []).map(s => ({ ...s, _subType: 'tempmail' })),
...
async function handleAddCustom(e) {
    ...
    } else if (subType === 'web2') {
        serviceType = 'web2';
        config = {
            base_url: formData.get('web2_base_url') || 'https://web2.temp-mail.org',
            timeout: parseInt(formData.get('web2_timeout'), 10) || 30,
            max_retries: parseInt(formData.get('web2_max_retries'), 10) || 3
        };
...
async function editCustomService(id, subType) {
    try {
        ...
        } else if (resolvedSubType === 'web2') {
            document.getElementById('edit-web2-base-url').value = service.config?.base_url || 'https://web2.temp-mail.org';
            document.getElementById('edit-web2-timeout').value = service.config?.timeout || 30;
            document.getElementById('edit-web2-max-retries').value = service.config?.max_retries || 3;
