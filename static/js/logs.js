...
function applyCleanupDefaults(data) {
    if (logsState.cleanupDefaultsHydrated) {
        return;
    }

    const retentionDays = Number(data.retention_days || 0);
    const maxRows = Number(data.max_rows || 0);

    if (retentionDays > 0) {
        el.cleanupRetentionDays.value = String(retentionDays);
    }
    if (maxRows > 0) {
        el.cleanupMaxRows.value = String(maxRows);
    }

    if (retentionDays > 0 || maxRows > 0) {
        logsState.cleanupDefaultsHydrated = true;
    }
}
...
async function cleanupLogs() {
    const retentionDays = Number(el.cleanupRetentionDays.value || 14);
