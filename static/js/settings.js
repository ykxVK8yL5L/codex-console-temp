...
const poolKeyMap = {
    ...
    max_overflow: "峰值补充",
    pool_timeout: "超时 (秒)",
    pool_recycle: "Recycle (秒)",
    pool_use_lifo: "LIFO 排队",
};

function formatPoolInfo(settings) {
    if (!settings || typeof settings !== 'object') {
        return '默认';
    }
    const entries = [];
    for (const [key, label] of Object.entries(poolKeyMap)) {
        if (key in settings) {
            entries.push(`${label}: ${settings[key]}`);
        }
    }
    return entries.length ? entries.join(' | ') : '默认';
}

function toggleSqliteButtons(enabled) {
    if (elements.backupBtn) {
        elements.backupBtn.disabled = !enabled;
    }
    if (elements.importDbBtn) {
        elements.importDbBtn.disabled = !enabled;
    }
    if (elements.dbImportFile) {
        elements.dbImportFile.disabled = !enabled;
    }
    const status = document.getElementById('db-file-status');
    if (status) {
        status.textContent = enabled
            ? '数据库文件操作当前仅支持 SQLite。'
            : '当前数据库为非 SQLite，不支持在线备份/导入，请使用外部工具。';
    }
}
...
async function loadDatabaseInfo() {
    try {
        ...
        const sizeLabel = document.getElementById('db-size');
        if (sizeLabel) {
            sizeLabel.textContent = data.database_size_label || `${data.database_size_mb} MB`;
        }
        ...
        const backendEl = document.getElementById('db-backend');
        if (backendEl) {
            backendEl.textContent = data.database_backend || '未知';
        }
        const poolEl = document.getElementById('db-pool');
        if (poolEl) {
            poolEl.textContent = formatPoolInfo(data.connection_pool);
        }
        toggleSqliteButtons(Boolean(data.database_supports_file_backup));
...
async function handleCleanup() {
    const confirmed = await confirm('确定要清理过期数据吗？默认会清理 14 天前的已完成/已取消任务、45 天前的失败任务、7 天前的待处理任务，以及 3 天前仍未结束的运行任务。此操作不可恢复。');
    if (!confirmed) return;
    ...
    try {
        const data = await api.post('/settings/database/cleanup');
...
function renderCodex2apiServicesTable(services) {
    if (!services || services.length === 0) {
        elements.codex2apiServicesTable.innerHTML = '<tr><td colspan="5" style="text-align:center;color:var(--text-muted);padding:20px;">暂无 Codex2API 服务，点击「添加服务」新增</td></tr>';
        return;
    }
...
function openCodex2apiServiceModal(service = null) {
    ...
    document.getElementById('codex2api-service-key').placeholder = service && service.has_key ? '已配置，留空保持不变' : '请输入 Admin Key';
...
async function handleSaveCodex2apiService(e) {
    ...
    const adminKey = document.getElementById('codex2api-service-key').value.trim();
...
async function handleTestCodex2apiService() {
    ...
    try {
        ...
        if (id && !adminKey) {
            ...
            result = await api.post('/codex2api-services/test-connection', { api_url: apiUrl, admin_key: adminKey });
        }
        if (result.success) {
            toast.success(result.message);
        } else {
