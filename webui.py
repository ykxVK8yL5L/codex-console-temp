...
from src.database.session import get_database_backend, get_database_pool_settings
...
def _find_available_port(host: str, preferred_port: int, max_scan: int = 100) -> int:
    """
    从首选端口开始向后查找可用端口。
    直接按实际监听地址探测，避免 0.0.0.0 与 127.0.0.1 探测结果不一致。
    """
...
def setup_application():
    ...
    logger.info("数据库后端: %s", database_backend)
    if database_backend != "sqlite":
        logger.info(
            "数据库连接池: size=%s overflow=%s timeout=%ss recycle=%ss lifo=%s pre_ping=%s",
            pool_settings.get("pool_size"),
            pool_settings.get("max_overflow"),
            pool_settings.get("pool_timeout"),
            pool_settings.get("pool_recycle"),
            pool_settings.get("pool_use_lifo"),
            pool_settings.get("pool_pre_ping"),
        )
...
def start_webui():
    ...
    display_host = _get_display_host(bind_host)
    ...
    logger.info(f"Web UI 已就位，请走这边: http://{display_host}:{selected_port}")
...
def main():
    ...
    parser.add_argument("--host", help="监听主机 (也可通过 APP_HOST / WEBUI_HOST 环境变量设置)")
    parser.add_argument("--port", type=int, help="监听端口 (也可通过 APP_PORT / WEBUI_PORT 环境变量设置)")
    ...
    parser.add_argument("--access-password", help="Web UI 访问密钥 (也可通过 APP_ACCESS_PASSWORD / WEBUI_ACCESS_PASSWORD 环境变量设置)")
